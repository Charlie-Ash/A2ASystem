...
class RAGTool():
    ...
    def run(self, tool_args):
        ...
        if not query:
            return "RAG tool error: no query was provided."
        ...
        try:
            ...
            return f"RAG tool error: generation failed ({e})"

        answer = outputs[0].outputs[0].text.strip()

        return (
            f"[RAG tool completed] The document Q&A answer below was already fully "
            f"generated by the RAG tool's own model ({LLM_MODEL}). Relay it to the user "
            f"as-is without rewriting or summarizing it, then ask what they'd like help "
            f"with next.\n\n{answer}"
        )
